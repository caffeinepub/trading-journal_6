    ...
    <div className="glass-card rounded-2xl p-5 flex items-center gap-4">
      <div
        className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${
          neutral ? "bg-primary/15" : positive ? "bg-profit/15" : "bg-loss/15"
        }`}
      >
        <Icon
          size={22}
          className={
            neutral ? "text-primary" : positive ? "text-profit" : "text-loss"
          }
        />
      </div>
      <div>
  ...
  const data = useMemo(() => {
    ...
    for (const t of trades) {
      ...
      const idx = days.findIndex(
        (d) => d.date.getTime() === tradeDate.getTime(),
      );
      if (idx !== -1) {
        days[idx].pnl += t.pnl;
      }
    ...
    return days.map((d) => ({
      label: d.label,
      pnl: Number.parseFloat(d.pnl.toFixed(2)),
    }));
  }, [trades]);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="glass-card-strong rounded-xl px-3 py-2 text-sm">
          ...
          <p
            className={
              val >= 0 ? "text-profit font-semibold" : "text-loss font-semibold"
            }
          >
            {formatCurrency(val)}
          </p>
              ...
              {monthlyStats.map((m) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span>
                      {m.total > 0 ? ((m.wins / m.total) * 100).toFixed(0) : 0}%
                      win
                    </span>
                  </div>
                </div>
