...
function RiskCalculator({
  ...
  const riskPerShare =
    entry > 0 && stopLoss > 0 ? Math.abs(entry - stopLoss) : 0;
  const riskAmount =
    entry > 0 && stopLoss > 0 && quantity > 0 ? riskPerShare * quantity : 0;
  const risk = entry > 0 && stopLoss > 0 ? Math.abs(entry - stopLoss) : 0;
  const reward = entry > 0 && target > 0 ? Math.abs(target - entry) : 0;
  const rrRatio = risk !== 0 ? reward / risk : 0;

  const riskClass = riskAmount > 0 ? "text-loss" : "text-muted-foreground";
  const rrClass =
    rrRatio >= 2
      ? "text-profit"
      : rrRatio >= 1
...
export default function NewTradePage() {
  ...
  const tgt = Number.parseFloat(target) || 0;
  const qty = Number.parseInt(quantity) || 0;

  const livePnl =
    entry > 0 && exit > 0 && qty > 0
      ? calcPnl(direction, entry, exit, qty)
      : null;
  const pnlColorClass =
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Record your trade details and psychology
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="glass-card rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label htmlFor="trade-market-type" className={labelClass}>
                Market Type
              </label>
              <select
          ...
          <div>
            <label htmlFor="trade-stock-name" className={labelClass}>
              Stock / Symbol
            </label>
            <input
              id="trade-stock-name"
        ...
        <div className="glass-card rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="trade-entry-price" className={labelClass}>
                Entry Price
              </label>
              <input
            ...
            <div>
              <label htmlFor="trade-exit-price" className={labelClass}>
                Exit Price
              </label>
              <input
                id="trade-exit-price"
            ...
            <div>
              <label htmlFor="trade-stop-loss" className={labelClass}>
                Stop Loss
              </label>
              <input
                id="trade-stop-loss"
            ...
            <div>
              <label htmlFor="trade-target" className={labelClass}>
                Target
              </label>
              <input
                id="trade-target"
            ...
            <div>
              <label htmlFor="trade-quantity" className={labelClass}>
                Quantity
              </label>
              <input
                id="trade-quantity"
            ...
            <div>
              <p className={labelClass}>Live P&amp;L</p>
              <div
                className={`${inputClass} flex items-center ${pnlColorClass} font-bold`}
              >
                {livePnl !== null
        ...
        <div className="glass-card rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="trade-emotion" className={labelClass}>
                Emotion
              </label>
              <select
                id="trade-emotion"
            ...
            <div>
              <label htmlFor="trade-mistake-type" className={labelClass}>
                Mistake Type
              </label>
              <select
                id="trade-mistake-type"
                value={mistakeType}
          ...
          <div>
            <label htmlFor="trade-strategy" className={labelClass}>
              Strategy
            </label>
          ...
          <div>
            <p className={labelClass}>Conviction Level</p>
            <ConvictionMeter
