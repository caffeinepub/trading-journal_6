...
export default function RiskManagementPage() {
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      if (accountInput)
        promises.push(
          setAccountSize.mutateAsync(Number.parseFloat(accountInput)),
        );
      if (maxLossInput)
        promises.push(
          setDailyMaxLoss.mutateAsync(Number.parseFloat(maxLossInput)),
        );
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Monitor your risk exposure and account settings
        </p>
      </div>

      {/* Live P&L Status */}
      <div
        className={`glass-card rounded-2xl p-5 border ${isOverLimit ? "border-loss/40" : "border-white/8"}`}
      >
        ...
        {riskLoading ? (
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-3 rounded-xl bg-white/3 border border-white/5">
              ...
              <p
                className={`text-lg font-bold font-display ${pnl >= 0 ? "text-profit" : "text-loss"}`}
              >
                {pnl >= 0 ? "+" : ""}
                {formatCurrency(pnl)}
              </p>
              <p
                className={`text-xs mt-0.5 ${pnlPercent >= 0 ? "text-profit" : "text-loss"}`}
              >
                {pnlPercent >= 0 ? "+" : ""}
            ...
            <div className="text-center p-3 rounded-xl bg-white/3 border border-white/5">
              ...
              <p className="text-lg font-bold font-display text-foreground">
                {formatCurrency(accSize)}
              </p>
              <p
                className={`text-xs mt-0.5 ${isOverLimit ? "text-loss font-semibold" : "text-profit"}`}
              >
      ...
      <div className="glass-card rounded-2xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="account-size" className={labelClass}>
              Account Size (₹)
            </label>
            <input
              id="account-size"
          ...
          <div>
            <label htmlFor="daily-max-loss" className={labelClass}>
              Daily Max Loss (%)
            </label>
            <input
              id="daily-max-loss"
